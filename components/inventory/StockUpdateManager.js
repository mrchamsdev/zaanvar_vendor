...
const StockUpdateManager = ({ onClose, initialData }) => {
  ...
  const [tabs, setTabs] = useState(() => {
    // Default single tab for Stock Update
    return [
      { 
        id: '1', 
        title: 'Stock Update 1', 
        isMinimized: false, 
        data: initialData || {}
      }
    ];
  ...
  const addTab = () => {
    ...
    const newTab = { id: newId, title: `Stock Update #${tabs.length + 1}`, isMinimized: false, data: {} };
    setTabs([...tabs, newTab]);
        ...
        {!splitMode ? (
          ...
          <>
            <div className={styles.formWrapper} style={{ padding: 0 }}>
              ...
              ) : <div style={{display:'flex', alignItems:'center', justifyContent:'center', height:'100%', color:'#888'}}>Select session</div>}
            </div>
            <div className={styles.formWrapper} style={{ padding: 0 }}>
