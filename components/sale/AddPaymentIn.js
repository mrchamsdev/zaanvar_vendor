    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (res && (res.status === "success" || res.data?.status === "success")) {
                ...
                // WhatsApp message prompt (only for new payments, not edits)
                if (mode !== 'edit') {
                    try {
                        ...
                        const sendMsg = msgSettings?.sendPaymentInMessageToCustomer;
                        const autoMessageEvents = msgSettings?.autoMessageEvents;
                        const isAutoEnabled = Array.isArray(autoMessageEvents)
                            ? autoMessageEvents.includes("Payment in")
                            : !!autoMessageEvents?.paymentIn;
                ...
                <div style={{
                    ...
                    <div style={{
                        ...
                        <p style={{ margin: "0 0 24px", fontSize: 14, color: "#666", lineHeight: 1.5 }}>
                            Do you want to send the payment in message to the customer?
                        </p>
