...
import { getTermsAndConditions, getSettings, triggerVendorMessage } from "../../services/settingsService";
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (res.status === "success" || res.status === 200) {
                ...
                // WhatsApp message prompt (only for new sales, not edits)
                if (mode !== "edit") {
                    try {
                        ...
                        const sendMsg = msgSettings?.sendSaleInvoiceMessageToCustomer;
                        const autoMessageEvents = msgSettings?.autoMessageEvents;
                        const isAutoEnabled = Array.isArray(autoMessageEvents)
                            ? autoMessageEvents.includes("Sale invoice")
                            : !!autoMessageEvents?.saleInvoice;
                ...
                <div style={{
                    ...
                    <div style={{
                        ...
                        <p style={{ margin: "0 0 24px", fontSize: 14, color: "#666", lineHeight: 1.5 }}>
                            Do you want to send the sale invoice message to the customer?
                        </p>
