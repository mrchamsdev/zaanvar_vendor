...
import { getTaxGroups, getTaxRates, getSettings, triggerVendorMessage } from "../../services/settingsService";
                            ...
                            if (matchingGroup) {
                                ...
                                if (taxIncluded && taxRate > 0 && batch.originalCostPrice !== undefined && batch.originalCostPrice !== "") {
                                    ...
                                    if (!isNaN(original) && original > 0) {
                                        const taxExclusive = original - (original * taxRate / 100);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (res.status === "success" || res.status === 200 || res.id) {
                ...
                try {
                    const activeBranchId = orderData?.branchId;
                    const settingsRes = await getSettings(jwtToken, activeBranchId);
                    ...
                    const sendMsg = msgSettings?.sendReceivedOrderMessageToSupplier;
                    const autoMessageEvents = msgSettings?.autoMessageEvents;
                    const isAutoEnabled = Array.isArray(autoMessageEvents)
                        ? autoMessageEvents.includes("Purchase order receive")
                        : !!autoMessageEvents?.purchaseOrderReceive;
                    ...
                    if (sendMsg && !isAutoEnabled) {
                        ...
                        if (userChoice) {
                            let matchingMsg = responseMessages.find(m => m.eventType === "purchaseOrderReceive");
                            if (!matchingMsg) {
                                await new Promise(r => setTimeout(r, 600));
                                const refetched = await getSettings(jwtToken, activeBranchId);
                                const refetchedMsgs = refetched?.data?.messages || refetched?.messages || [];
                                matchingMsg = refetchedMsgs.find(m => m.eventType === "purchaseOrderReceive");
                            }
            ...
            {messageModal.isOpen && (
                <div style={{
                    ...
                    <div style={{
                        ...
                        <h3 style={{ margin: "0 0 8px", fontSize: 18, fontWeight: 600, color: "#111" }}>Send Message to Supplier?</h3>
                        <p style={{ margin: "0 0 24px", fontSize: 14, color: "#666", lineHeight: 1.5 }}>
                            Do you want to send the received order message to the supplier?
                        </p>
