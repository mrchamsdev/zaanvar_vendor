          ...
          {visibleCards.map((category, index) => (
            ...
            >
              <div 
                ...
                onClick={() => router.push(`/coming-soon/${category.title.toLowerCase()}`)}
                style={{ cursor: "pointer" }}
              >
                <div className={styles.imageContainer}>
                  <img src={category.img} alt={category.title} />
                  <div
                    className={`${styles.imageMask} ${index === 2 ? styles.maskActive : ""
                      }`}
                  ></div>
                </div>
        ...
        {Array.from({ length: 5 }).map((_, i) => {
          ...
          return (
            <span
              ...
              onClick={() => {
                setCurrentIndex(idx);
                // restart interval so it doesn't immediately slide after click
                startAutoPlay();
              }}
              style={{ cursor: "pointer" }}
